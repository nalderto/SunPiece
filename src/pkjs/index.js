var myAPIKey = '0eb97c3103cccf27877cd04f5cbbc98c';



var xhrRequest = function (url, type, callback) {
  var xhr = new XMLHttpRequest();
  xhr.onload = function () {
    callback(this.responseText);
  };
  xhr.open(type, url);
  xhr.send();
};

function locationSuccess(pos) {
  // Construct URL
  var url = "http://api.openweathermap.org/data/2.5/weather?lat=" +
      pos.coords.latitude + "&lon=" + pos.coords.longitude + '&appid=' + myAPIKey;

  // Send request to OpenWeatherMap
  xhrRequest(url, 'GET', 
    function(responseText) {
      // responseText contains a JSON object with weather info
      var json = JSON.parse(responseText);
      var sstime = json.sys.sunset;
			var ssdate = new Date(sstime*1000);
			var sshours = ssdate.getHours();
			var ssminutes = "0" + ssdate.getMinutes();
			var sunset = (sshours - 12) + ':' + ssminutes.substr(-2);
			
      var srtime = json.sys.sunrise;
			var srdate = new Date(srtime*1000);
			var srhours = srdate.getHours();
			var srminutes = "0" + srdate.getMinutes();
			var sunrise = srhours + ':' + srminutes.substr(-2);
			
      console.log("Sunset is " + sunset);
			console.log("Sunrise is " + sunrise);
			console.log(pos.coords.latitude);
			console.log(pos.coords.longitude);
      // Assemble dictionary using our keys
      var dictionary = {
				"SUNRISE": sunrise,
        "SUNSET": sunset
      };

      // Send to Pebble
      Pebble.sendAppMessage(dictionary,
        function(e) {
          console.log("Sunrise/Sunset info sent to Pebble successfully!");
        },
        function(e) {
          console.log("Error sending sunrise/sunset info to Pebble!");
        }
      );
    }      
  );
}

function locationError(err) {
  console.log("Error requesting location!");
}

function getSunset() {
  navigator.geolocation.getCurrentPosition(
    locationSuccess,
    locationError,
    {timeout: 15000, maximumAge: 60000}
  );
}

// Listen for when the watchface is opened
Pebble.addEventListener('ready', 
  function(e) {
    console.log("PebbleKit JS ready!");

    // Get the initial sunset
    getSunset();
  }
);

// Listen for when an AppMessage is received
Pebble.addEventListener('appmessage',
  function(e) {
    console.log("AppMessage received!");
    getSunset();
  }                     
);
